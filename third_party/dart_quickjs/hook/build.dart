...
void main(List<String> args) async {
  await build(args, (input, output) async {
    ...
    if (targetOS != OS.windows) {
      ...
      // OpenHarmony (ohos): the flutter-ohos toolchain builds native assets
      // with the OHOS clang from the HarmonyOS SDK, which needs an explicit
      // target triple and sysroot to find its musl headers.
      final ohosCc = input.config.code.cCompiler?.compiler.toFilePath() ?? '';
      if (ohosCc.toLowerCase().contains('openharmony')) {
        final ccPath = ohosCc.replaceAll(r'\', '/');
        final llvmIdx = ccPath.indexOf('/llvm/bin/');
        if (llvmIdx > 0) {
          final ohosTarget = switch (targetArch) {
